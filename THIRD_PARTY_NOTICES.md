...
확인일: 2026-09-08. 대상: 이 저장소의 소스, requirements.txt, Windows 빌드·패키징 스크립트. **오픈소스 저작권 라이선스상 소스 공개는 가능하다. 아직 만들어지지 않은 설치 파일 전체의 배포 조건까지 검증 완료한 것은 아니다.**
...
1. 실제 설치 EXE에 들어간 DLL/플러그인/Python 패키지 목록과 버전을 확정한다. 빌드 스크립트가 설치된 모든 DLL을 복사하므로 현재는 필요한 것보다 많이 포함된다.
...
5. 검사되지 않은 설치 파일을 배포 완료본으로 표시하지 않는다. 현재 GitHub workflow는 빌드만 검사하고 설치 EXE를 자동 공개 아티팩트로 업로드하지 않는다.
