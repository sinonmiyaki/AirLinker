...
현재 빌드는 의존성 누락 방지를 위해 UCRT64의 DLL과 플러그인 전체를 포함합니다. 크기 최적화는 하지 않았습니다. `runtime/packages.txt`와 `runtime/share/licenses`를 남깁니다. Inno Setup 설치 프로그램 정의와 설치 EXE 생성 단계를 추가했습니다. 실제 Windows 컴파일은 아직 실행하지 않았습니다. 슬림 패키지, 코드 서명, 자동 업데이트는 후속 작업입니다. MSYS2 패키지는 설치 시점 버전을 사용하므로 엔진 소스만 커밋으로 고정되어 있고 전체 툴체인이 재현 가능하게 고정된 것은 아닙니다.
