...
v0.1.2에서는 프레임 출력 후 상단 바가 180ms 동안 위로 접히고 영상 영역이 빈 공간을 채웁니다. 상단 중앙 220px 폭·16px 높이에 커서가 들어오면 버튼이 내려옵니다. 열린 바는 커서가 버튼 영역에 있는 동안 유지하며, 설정 대화상자를 닫은 뒤 커서가 떠나면 다시 접힙니다. 연결 대기 상태에서는 항상 표시합니다.

영상 창은 다른 프로세스가 소유하므로 Qt mouseMoveEvent에 의존하지 않고 재생 중에만 50ms 간격으로 커서를 확인합니다. Windows의 실제 창 hit-test로 다른 앱에 가려진 경우를 제외하고, Win32 좌표는 GetCursorPos로 읽어 Qt 논리 좌표와 고배율 모니터의 물리 좌표를 섞지 않습니다.
