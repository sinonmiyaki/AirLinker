...
- 비행기 로고: `airlinker/assets/*.svg`에 직접 작성한 벡터 도형. PNG·ICO·BMP는 `scripts/build-icons.py`로 생성하며 프로젝트와 같은 GPL-3.0-or-later로 제공한다. 외부 로고·스톡 에셋을 사용하지 않았다.
...
| OpenSSL / libplist / 기타 MSYS2 DLL | 버전·패키지별 조건 | 선택한 플러그인의 PE DLL 의존성을 수집. 실제 패키지 목록과 대응 소스 확보 필요 |
