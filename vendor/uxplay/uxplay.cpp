/* Modified for AirLinker on 2026-09-08: native window embedding and lifecycle events.
 * See AIRLINKER-UPSTREAM.txt for the original revision. */
/**
...
extern "C" void conn_destroy (void *cls) {
    ...
    if (open_connections == 0) {
        fprintf(stdout, "AIRLINKER/1 IDLE\n");
        remote_clock_offset = 0;
        if (use_audio) {
            audio_renderer_stop();
        }
...
#define PID_MAX 4194304 // 2^22
    ...
    fprintf(stdout, "AIRLINKER/1 READY\n");
    reconnect:
    compression_type = 0;
